...
public final class Promise<Value> {
	...
	@discardableResult
	public func finally(on queue: DispatchQueue = .main, _ onFinally: @escaping () -> Void) -> Self {
		self.finallyCallbacks.append(onFinally)
		return self
	}
    ...
    private func fireCallbacksIfCompleted() {
        lockQueue.async(execute: {
			...
			self.finallyCallbacks.forEach { callback in
				callback()
			}
			self.finallyCallbacks.removeAll()
		})
    }
}
